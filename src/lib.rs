...
pub async fn run(file: impl AsRef<Path>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for balance in state.accounts.into_values() {
        println!(
            "{},{},{},{},{}",
            balance.client_id,
            format_decimals(balance.available),
            format_decimals(balance.held),
            format_decimals(balance.total),
            balance.locked
        );
    }
...
fn format_decimals(value: f32) -> String {
